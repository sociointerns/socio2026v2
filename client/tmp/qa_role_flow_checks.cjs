...
const envCandidates = [
  {
    client: path.resolve(__dirname, "..", ".env"),
    server: path.resolve(__dirname, "..", "..", "server", ".env"),
  },
  {
    client: path.resolve(__dirname, "..", "client", ".env"),
    server: path.resolve(__dirname, "..", "server", ".env"),
  },
  {
    client: path.resolve(process.cwd(), ".env"),
    server: path.resolve(process.cwd(), "..", "server", ".env"),
  },
];
...
for (const candidate of envCandidates) {
  const nextClientEnv = parseEnvFile(candidate.client);
  const nextServerEnv = parseEnvFile(candidate.server);

  if (Object.keys(nextClientEnv).length > 0 || Object.keys(nextServerEnv).length > 0) {
    clientEnv = nextClientEnv;
    serverEnv = nextServerEnv;
    break;
  }
}
...
async function createAuthAndProfile({ email, password, name, scopedRole, isMasterAdmin = false, campus = null }) {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const missingColumn = extractMissingColumn(upsertResult.error.message);

    if (!missingColumn || !Object.prototype.hasOwnProperty.call(profile, missingColumn)) {
      break;
    }

    delete profile[missingColumn];
  }
